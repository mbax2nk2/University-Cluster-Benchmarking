...
def main():
	...
	if(option == "mean"):
		for i in range(len(cluster_names)):
			...
			proc=subprocess.Popen(["Rscript", "/home/mbax2nk2/nursultan/Library/rank_generate_heatmap.R","../Graphs/"+clusterName+"/", option+".csv", option+".png", option])	
			proc.communicate()
	if(option == "lowest"):
		for i in range(len(cluster_names)):
			clusterName = cluster_names[i]
...
def getBenchmarksDictionary(lastLine):
	...
	dictionary = {'check' : check, 'startup.helloworld' : startupHelloWorld, 'startup.compiler.compiler' : startupCompilerCompiler, 'startup.compiler.sunflow' : startupCompilerSunflow, 'startup.compress' : startupCompress, 'startup.crypto.aes' : startupCryptoAes, 'startup.crypto.rsa' : startupCryptoRsa, 'starrtup.crypto.signverify' : startupCryptoSignverify, 'startup.mpegaudio' : startupMpegaudio, 'startup.scimark.fft' : startupScimarkFFT, 'startup.scimark.lu' : startupScimarkLu, 'startup.scimark.monte_carlo' : startupScimarkMonteCarlo, 'startup.scimark.sor' : startupScimarkSor, 'startup.scimark.sparse' : startupScimarkSparse, 'startup.serial' : startupSerial, 'startup.sunflow' : startupSunflow, 'startup.xml.transform' : startupXmlTransform, 'startup.xml.validation' : startupXmlValidation, 'compiler.compiler' : compilerCompiler, 'compiler.sunflow' : compilerSunflow, 'compress' : compress, 'crypto.aes' : cryptoAes, 'crypto.rsa' : cryptoRsa, 'crypto.signverify' : cryptoSignverify, 'derby' : derby, 'mpegaudio' : mpegaudio, 'scimark.fft.large' : scimarkFFT, 'scimark.lu.large' : scimarkLuLarge, 'scimark.sor.large' : scimarkSorLarge, 'scimark.sparse.large' : scimarkSparseLarge, 'scimark.fft.small' : scimarkFFtSmall, 'scimark.lu.small' : scimarkLuSmall, 'scimark.sor.small' : scimarkSorSmall, 'scimark.sparse.small' : scimarkSparseSmall, 'scimark.monte_carlo' : scimarkMonteCarlo, 'serial' : serial, 'sunflow' : sunflow, 'xml.transform' : xmlTransform, 'xml.validation' : xmlValidation}	
	return dictionary
main()		
